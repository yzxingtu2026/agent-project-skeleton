...
`.agent/tools/agent-rules/cli.js` 从中立元结构生成各 AI 编程工具可识别的规则和技能文件，避免手工维护多套重复目录。
...
```bash
node .agent/tools/agent-rules/cli.js sync --target=cursor,qoder
```
...
- 生成文件顶部带 `Generated by .agent/tools/agent-rules` 标记。
