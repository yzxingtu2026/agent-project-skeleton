...
模板内置一套可选的 GitHub Actions 钉钉协作通知能力，用于把 Issue/PR 分配、评论、审核和 main 分支更新事件推送到项目钉钉群，并按 `.agent/team/members.yml` 自动 @ 对应成员。
...
默认不通知单纯的新建 Issue / 新建 PR，也不通知关闭、重开、里程碑等普通状态变化，避免把钉钉群刷成 GitHub 流水账。Issue/PR 生命周期事件只在 assign 或 request review 这类有明确处理对象时推送。

协作通知会聚合相关人并在同一条消息内 @：评论正文中的已绑定 `@github-login`、Issue/PR assignees、PR requested reviewers、Issue/PR 作者。评论、审核、作者等间接相关人会去重并排除当前事件操作者本人；直接 assign 或 request review 的明确目标人会保留 @，即使操作者把任务分配给自己。
...
main 分支更新由 `.github/workflows/dingtalk-notify-main-updated.yml` 单独通知，会 @ `.agent/team/members.yml` 中所有配置了 `dingtalk.user_id` 的成员，提醒大家拉取最新代码。
...
- main 分支有新提交后，钉钉群收到更新通知并 @ 全部已绑定成员，提醒拉取最新代码。
