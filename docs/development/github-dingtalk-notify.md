...
模板内置一套可选的 GitHub Actions 钉钉协作通知能力，用于把 Issue、PR、评论和 Review 事件推送到项目钉钉群，并按 `.agent/team/members.yml` 自动 @ 对应成员。
...
- 新建 PR 后钉钉群收到普通通知。
- 请求已绑定成员 Review PR 后，钉钉群收到通知并 @ 对应成员。
- 短时间内批量 assign / request review 时，同一目标人收到一条汇总通知，而不是多条刷屏通知。
- Issue 评论、PR 评论、PR Review、PR 行评论新增时，钉钉群收到摘要与链接。
