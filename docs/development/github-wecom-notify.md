...
企业微信群机器人只能发送消息，不能读取群成员列表，也不能根据群昵称、手机号或 GitHub login 反查 userid。userid 需要由企业管理员从企业通讯录获取。

推荐优先使用管理员后台人工查询：

1. 企业管理员登录企业微信管理后台。
2. 进入“通讯录”，打开对应成员详情。
3. 读取成员的“账号”字段，该字段即企业内部 userid。
4. 将账号原样填写到 `notifications.wecom.user_id`，注意大小写和字符必须完全一致。

如果需要批量维护，可由具备通讯录权限的企业微信自建应用调用通讯录 API 获取成员详情或部门成员列表。该方式需要 CorpID、应用 Secret 和通讯录可见范围，不建议仅为少量成员引入自动同步。

不要填写群昵称、姓名、手机号、邮箱、微信号或 GitHub login。配置后应通过一条分配或审核请求通知验证 `<@userid>` 是否能在目标群内正确提醒该成员。
