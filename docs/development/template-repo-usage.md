...
3. 运行 `node .agent/tools/agent-rules/cli.js sync` 生成规则和技能文件。
...
- 首次使用或更换操作者时运行 `node .agent/tools/agent-rules/cli.js init`。
- 生成物过期时运行 `node .agent/tools/agent-rules/cli.js sync`，提交前可运行 `node .agent/tools/agent-rules/cli.js doctor`。
